...
class HETEGNNExplainer(torch.nn.Module):
    ...
    def __set_masks__(self, x, edge_index, edge_masks=None, edge_mask=None):
        ...
        # 初始化特征反事实用到的矩阵
        # 特征遮盖：Mb矩阵
        self.masking_matrices = torch.FloatTensor(N, F) * std
        torch.nn.init.xavier_uniform_(self.masking_matrices.data, gain=5 / 3)
    ...
    # 计算 similarity_loss 和 kl_div 的原始代码
    ...
    def explain_node(self, node_idx, x, edge_index, classifier, hete_graph, want_type, test_idx_dic, hyper_graph,
        ...
        if sub_hete_graph[0]:
            ...
            for epoch in range(1, self.epochs + 1):
                ...
                loss = self.__loss__(mapping, res, log_logits, pred_label, x, masking_matrices)
