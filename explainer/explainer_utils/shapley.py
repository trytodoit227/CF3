...
def GnnNetsNC2valueFunc(gnnNets_NC, hyp_graph, classifier, node_idx, target_class):
    def value_func(data):
        # 获得异质图
        edge_index = data.edge_index.transpose(0, 1).tolist()
        edge_type = data.pos.transpose(0, 1).tolist()[0]
        edge_weight = data.edge_attr.transpose(0, 1).tolist()[0]
        hete_graph = [edge_index, edge_type, edge_weight]
        # 获得超图、idx
        idx = list(i.item() for i in data.y)
        with torch.no_grad():
            company_emb = gnnNets_NC(hete_graph=hete_graph, hyp_graph=hyp_graph, idx=idx, x=data.x.float())
            probs = classifier.forward(company_emb).reshape(-1, 2)
...
class MarginalSubgraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        exclude_data = Data(x=exclude_graph_X, edge_index=exclude_graph_edge_index, edge_attr=self.edge_attr,
                            pos=self.edge_type, y=self.nodes)
...
def marginal_contribution(data, exclude_mask, include_mask, value_func, subgraph_build_func, ):
    ...
    # exclude_data 和 include_data 的 edge_index 是完全一样的
    for exclude_data, include_data in dataloader:

        exclude_data.edge_index = data.edge_index
        exclude_data.edge_attr = data.edge_attr
        exclude_data.pos = data.pos
        exclude_values = value_func(exclude_data)

        include_data.edge_index = data.edge_index
        include_data.edge_attr = data.edge_attr
        include_data.pos = data.pos
