...
class HETEGNNExplainer(torch.nn.Module):
    ...
    def generator(self, node_feature_mask, edge_index, hyper_graph, sub_hete_graph, sub_hyp_graph):
        ...
        dis_perturbation_adjs = torch.ones(perturbation_adjs_sparse.to_dense().shape[0],
                                           perturbation_adjs_sparse.to_dense().shape[
                                               1]) - perturbation_adjs_sparse.to_dense()
        ...
        return adjs_dense, perturbation_adjs, p_hete_graph, p_hyp_graph, masking_matrices, masked_attrs, dis_masked_attrs, dis_p_hete_graph, dis_p_hyp_graph
    ...
    def similarity_loss(self, adjs_dense, perturbation_adjs, masking_matrix):
        ...
        diff_norm = torch.linalg.matrix_norm(masking_matrix, ord=1) / torch.ones_like(masking_matrix).sum()
    ...
    def kl_div_target(self, predicted_results, perturbation_predicted_results, combine_results,
                      masking_predicted_results):
        # predicted_results = predicted_results.softmax(dim=1)
        ...
        combine_predicted_results = combine_results.log_softmax(dim=1)
        loss_func = torch.nn.KLDivLoss(reduction='batchmean')
        kl_loss_1 = loss_func(perturbation_predicted_results, predicted_results)
        kl_loss_2 = loss_func(masking_predicted_results, predicted_results)
        kl_loss_3 = loss_func(combine_predicted_results, predicted_results)
    ...
    # raw
    def __loss__(self, node_idx, res, log_logits_p, log_logits_m, log_logits_mp, log_logits_count, pred_label, adjs_dense,
                 perturbation_adjs, masking_matrix):
        pre_loss = -log_logits_p[node_idx, pred_label] - log_logits_m[node_idx, pred_label] - log_logits_mp[
            node_idx, pred_label]-log_logits_count[node_idx, 1-pred_label]  # 获得 node_idx 的损失
        ...
        # target_results = torch.FloatTensor([[0, 1]]) if pred_label.item() == 1 else torch.FloatTensor([[1, 0]])
        # kl_loss = self.kl_div_target(target_results, log_logits_p[node_idx], log_logits_m[node_idx],
        #                              log_logits_mp[node_idx])

        # kl_loss = self.kl_div(res, log_logits_p, log_logits_m, log_logits_mp)
        # loss1 += (self.coeffs['weight_loss'] * sim_loss + (1 - self.coeffs['weight_loss']) * kl_loss)

        loss1 = pre_loss * self.weight_loss + ms_entropy * self.weight_loss1 + self.weight_loss2 * sim_loss

        # loss1 =  ms_entropy * self.weight_loss1 + self.weight_loss2 * sim_loss -  self.weight_loss3 * kl_loss
        ...
        if sub_hete_graph[0]:
            ...
            optimizer = torch.optim.Adam(
                [self.perturbation_matrices, self.perturbation_biases, self.masking_matrices, self.edge_mask,
                 self.weight_loss, self.weight_loss1, self.weight_loss2, self.weight_loss3],
                lr=self.lr)
            ...
            for epoch in range(1, self.epochs + 1):
                ...
                masked_attrs, dis_masked_attrs, dis_p_hete_graph, dis_p_hyp_graph = self.generator(x, edge_index, hyper_graph, sub_hete_graph, sub_hyp_graph)
                # 边扰动构成的事实
                ...
                # loss = self.__loss__(mapping, res,  log_logits_conbine, pred_label, adjs_dense,
                #                      perturbation_adjs, masked_attrs)
                # loss = self.__loss__(mapping, res.detach(), log_logits_p, log_logits_m, log_logits_conbine, pred_label,
                #                      adjs_dense, perturbation_adjs, masked_attrs)

                # #factual an counterfactual
                #
                # # 边扰动构成的事实
                # company_emb_p = self.model(hete_graph=p_hete_graph, hyp_graph=p_hyp_graph, idx=sub_idx,
                #                            x=x.float())
                ...
                # # loss = self.__loss__(mapping, res,  log_logits_conbine, pred_label, adjs_dense,
                # #                      perturbation_adjs, masked_attrs)
                loss = self.__loss__(mapping, res.detach(), log_logits_p, log_logits_m, log_logits_conbine, log_logits_count, pred_label,
                                     adjs_dense, perturbation_adjs, masked_attrs)
