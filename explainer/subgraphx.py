...
from explainer.explainer_utils.shapley import (
    GnnNetsNC2valueFunc,
    NC_mc_l_shapley,
    gnn_score,
    l_shapley,
...
from utils.model_utils import gen_attribute_hg
...
def reward_func(reward_method, value_func, node_idx=None, local_radius=4, sample_num=100,
...
class MCTS(object):
    ...
    def __init__(
        ...
        self.data = Data(x=self.X, edge_index=self.edge_index, edge_attr=self.edge_attr, pos=self.edge_type)
        graph_data = Data(
            x=self.X,
        ...
        if node_idx is not None:
            ...
            x, edge_index, subset, _, edge_mask, kwargs = self.__subgraph__(node_idx, self.X,
                                                                            self.edge_index,
                                                                            self.test_idx_dic,
                                                                            self.num_hops,
                                                                            self.test_company_num)
            edge_attr = self.edge_attr[edge_mask]
            edge_type = self.edge_type[edge_mask]

            s = torch.tensor([i.item() for i in subset if i.item() < 3976
                              and i.item() in self.test_idx_dic.keys()
                              and self.test_idx_dic[i.item()] < self.test_company_num])
            s_index = torch.tensor([self.test_idx_dic[i.item()] for i in s])

            self.data = Batch.from_data_list(
                [Data(x=x, edge_index=edge_index, edge_attr=edge_attr, pos=edge_type, y=s_index)])
            self.graph = self.ori_graph.subgraph(s.tolist())
            mapping = {int(v): k for k, v in enumerate(s)}
            self.mapping_inv = {int(k): int(v) for k, v in enumerate(s)}
            self.graph = nx.relabel_nodes(self.graph, mapping)
            self.new_node_idx = list(s).index(node_idx)  # 重新编码
            self.num_nodes = self.graph.number_of_nodes()
    ...
    @staticmethod
    def __subgraph__(node_idx, x, edge_index, test_idx_dic, num_hops, test_company_num, **kwargs):
        num_nodes, num_edges = 6381, edge_index.size(1)
        subset, edge_index, mapping, edge_mask = k_hop_subgraph(node_idx, num_hops, edge_index, relabel_nodes=False,
        ...
        s = torch.tensor([i.item() for i in s if i.item() < test_company_num])
        x = x[s]
        ...
        return x, edge_index, subset, mapping, edge_mask, kwargs
    ...
    def mcts_rollout(self, tree_node):
        ...
        if len(tree_node.children) == 0:
            ...
            for each_node in expand_nodes:
                ...
                subgraphs = [self.graph.subgraph(c) for c in
                             nx.connected_components(self.graph.subgraph(subgraph_coalition))]
...
class SubgraphX(object):
    ...
    Args:
        model (:obj:`torch.nn.Module`): The target model prepared to z_explain_model
    ...
    def get_reward_func(self, value_func, node_idx=None):
        ...
        return reward_func(reward_method=self.reward_method, value_func=value_func, node_idx=node_idx,
                           local_radius=self.local_radius, sample_num=self.sample_num,
                           subgraph_building_method=self.subgraph_building_method, )

    def get_mcts_class(self, x, edge_index, edge_attr, edge_type, test_idx_dic, node_idx=None, score_func=None, ):
        assert node_idx is not None
        return MCTS(x, edge_index, edge_attr, edge_type, test_idx_dic, node_idx=node_idx, device=self.device,
                    score_func=score_func, num_hops=self.num_hops, n_rollout=self.rollout, min_atoms=self.min_atoms,
                    c_puct=self.c_puct, expand_atoms=self.expand_atoms, high2low=self.high2low,
                    test_company_num=self.test_company_num)
    ...
    def explain(self, x, edge_index, classifier, hete_graph, test_idx_dic, hyper_graph, label, max_nodes=6,
                node_idx=None, saved_MCTSInfo_list=None):
        hyp_graph = []
        ...
        edge_type = torch.tensor(hete_graph[1]).reshape(-1, 1)
        edge_attr = torch.tensor(hete_graph[2]).reshape(-1, 1)
        self.mcts_state_map = self.get_mcts_class(x, edge_index, edge_attr, edge_type, test_idx_dic, node_idx=node_idx)
        self.new_node_idx = self.mcts_state_map.new_node_idx
        # mcts will extract the subgraph and relabel the nodes
        value_func = GnnNetsNC2valueFunc(self.model, hyp_graph, classifier, node_idx=self.mcts_state_map.new_node_idx,
                                         target_class=label, )
        ...
        node_mask = torch.zeros(6381)
        node_mask[masked_node_list] = 1.0

        row, col = self.ori_data.edge_index
        edge_mask = (node_mask[col] == 1) & (node_mask[row] == 1)
        if len(edge_mask.nonzero()) == 0:
            # edge_mask = (node_mask[col] == 1) | (node_mask[row] == 1)
            edge_mask = (node_mask[col] == 1)  # 最终选择
            # edge_mask = (node_mask[row] == 1)
    ...
    def __call__(
        ...
        r"""z_explain_model the GNN behavior for the graph using SubgraphX method
        Args:
            ...
            kwargs(:obj:`Dict`):
              The additional parameters
                - node_idx (:obj:`int`, :obj:`None`): The target node index when z_explain_model node classification task
