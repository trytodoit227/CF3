...
from utils.model_utils import get_degree_matrix
from explainer.explainer_utils.modelperturb import MODELPerturb
...
class CFExplainer:
    ...
    def __init__(self, model, cf_model_name, adj, feat, hete_graph, hyp_graph, classifier, n_hid, dropout, readout,
                 edge_dim, num_layers, labels, y_pred_orig, beta, device, mapping):
        ...
        self.hyp_graph = hyp_graph  # 没经过处理的原始图，要使用需经过处理
        ...
        self.cf_model = eval(self.cf_model_name[0].upper() + "Perturb")(input_dim, output_dim, self.adj, 3976, 2405, 20,
                                                                        self.device, beta)
    ...
    def train_node(self, epoch):
        ...
        output = self.cf_model.forward_emb(
            **{"x": self.x, "adj": self.A_x, "hete_graph": self.hete_graph, "hyp_graph": self.hyp_graph,
               "classifier": self.classifier, "mapping": self.mapping})
        output_actual, self.P = self.cf_model.forward_emb_prediction(
            **{"x": self.x, "adj": self.A_x, "hete_graph": self.hete_graph, "hyp_graph": self.hyp_graph,
               "classifier": self.classifier, "mapping": self.mapping})
        ...
        # )
        (loss_total, loss_pred, loss_graph_dist, cf_adj) = self.cf_model.cf_loss(output[self.new_idx], self.y_pred_orig,
                                                                                 y_pred_new_actual)
        loss_total.backward()
        ...
        if y_pred_new_actual != self.y_pred_orig:
            ...
            perturb = np.abs(cf_adj.cpu().detach().numpy() - self.adj.cpu().detach().numpy())
