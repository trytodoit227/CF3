...
class CF3(torch.nn.Module):
    ...
    def __init__(self, model, num, temp, x_shape, batch_size, epochs, test_company_num, device,
                 feat_mask_type: str = "feature", lr: float = 0.01, num_hops: Optional[int] = None, log: bool = True):
    ...
    def explain_node(self, node_idx_list, x, edge_index, classifier, hete_graph, want_type, test_idx_dic, hyper_graph,
        ...
        for node_idx in node_idx_list:
            ...
            # 获得单个节点的所有处理后的子图
            node_subgraph = {'adjs': adjs, 'mm_adjs': mm_adjs, 'sub_hete_graph': sub_hete_graph,
                             'sub_hyp_graph': sub_hyp_graph, 'sub_idx': sub_idx, 'sub_x': sub_x,
                             'p_hete_graph': p_hete_graph, 'p_x': p_x, 'dis_p_hete_graph': dis_p_hete_graph,
                             'dis_p_x': dis_p_x, 'mapping': mapping}
            node_subgraphs.append(node_subgraph)

            # edge mask，重新赋值
            em = self.edge_mask.new_zeros(num_edges)
            ...
            if common_edge_index.numel() == 0:
                ...
                sem = torch.nn.Parameter(torch.randn(pem.nonzero().shape[0]) * self.std_e)
                em[pem] = sem
            ems += em
            sems.append(em)
            # node mask
            p_n = torch.sum(p_x, dim=0)
            with torch.no_grad():
                p_n.normal_(1.0, self.std_n)
            mas += p_n
            masking_matrices.append(p_n)
        ...
        #     pbar.set_description(f'Explain batches: {node_idx_list}')
        mas = (mas - torch.min(mas)) / (torch.max(mas) - torch.min(mas))
        self.masking_matrices = torch.nn.Parameter(mas)
        self.edge_mask = torch.nn.Parameter(ems)
        if is_training:
            ...
            for epoch in range(1, self.train_epochs + 1):
                ...
                for batch_idx in range(num_batches):
                    ...
                    # average_loss = losses / len(true_idx_batch)
                    average_loss = losses / len(range(start_idx, end_idx))
                    print(average_loss)
                    average_loss.backward()
                    optimizer.step()

                # true_idx.append(true_idx_batch)
        ...
        node_feat_mask = []
        for mm in masking_matrices:
            # nfm = (mm * self.masking_matrices).detach().sigmoid()
        ...
        edge_mask = []
        for sem in sems:
            # em = (sem * self.edge_mask).detach().sigmoid()
