...
class HETEGNNExplainer(torch.nn.Module):
    ...
    def generator(self, node_feature_mask, edge_index, hyper_graph):
        # 近距离扰动
    ...
    # raw
    def __loss__(self, node_idx, res, log_logits_p, log_logits_m, log_logits_mp, pred_label, adjs_dense,
                 perturbation_adjs, masking_matrix):
        # loss = -log_logits_p[node_idx, pred_label] - log_logits_m[node_idx, pred_label]  # 获得 node_idx 的损失
        loss1 = -log_logits_p[node_idx, 1 - pred_label] - log_logits_m[node_idx, 1 - pred_label] - log_logits_mp[
        ...
        loss1 += sum([self.coeffs['edge_size'] * edge_reduce(i) for i in ms])
        ents = [-i * torch.log(i + EPS) - (1 - i) * torch.log(1 - i + EPS) for i in ms]
        loss1 += sum([self.coeffs['edge_ent'] * i.mean() for i in ents])
    ...
    #     # masking_predicted_results = masking_predicted_results.softmax(dim=1)
    #     loss_func = torch.nn.KLDivLoss(reduction='batchmean')
    #     kl_loss_1 = loss_func(torch.log(perturbation_predicted_results), predicted_results)
    #     # kl_loss_2 = loss_func(masking_predicted_results, predicted_results)
    #     return kl_loss_1
    # #改动后的损失函数
    # def __loss__(self, node_idx, res, log_logits_conbine, pred_label, adjs_dense, perturbation_adjs,
    #              masking_matrix):
    #     # loss = -log_logits_p[node_idx, pred_label] - log_logits_m[node_idx, pred_label]  # 获得 node_idx 的损失
    #     # loss = -log_logits_p[node_idx, 1-pred_label] - log_logits_m[node_idx, 1-pred_label] #将预测标签改为1-pre_label
    #
    #
    #     # 增加了边的类型,求边的loss
    #     ms = [i.sigmoid() for i in self.edge_masks]
    #     edge_reduce = getattr(torch, self.coeffs['edge_reduction'])  # sum
    #     loss = sum([self.coeffs['edge_size'] * edge_reduce(i) for i in ms])
    #     ents = [-i * torch.log(i + EPS) - (1 - i) * torch.log(1 - i + EPS) for i in ms]
    #     loss = loss + sum([self.coeffs['edge_ent'] * i.mean() for i in ents])
    #
    #     # 求节点的loss，反事实
    #     sim_loss = self.similarity_loss(adjs_dense, perturbation_adjs, masking_matrix)
    #     kl_loss = self.kl_div(res, log_logits_conbine)
    #     # loss = loss + (self.coeffs['weight_loss']*sim_loss - kl_loss)#raw
    #
    #     loss = loss + self.coeffs['weight_loss'] * sim_loss +(1-self.coeffs['weight_loss']) * kl_loss
    #
    #     return loss
    ...
    def explain_node(self, node_idx, x, edge_index, classifier, hete_graph, want_type, test_idx_dic, hyper_graph,
        ...
        if sub_hete_graph[0]:
            ...
            for epoch in range(1, self.epochs + 1):
                optimizer.zero_grad()
                # 扰动后的结果
                adjs_dense, perturbation_adjs, p_hete_graph, p_hyp_graph, \
                    masking_matrices, masked_attrs = self.generator(x, edge_index, hyper_graph)
                # 边扰动构成的反事实
