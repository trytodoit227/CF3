...
def k_hop_subgraph_with_default_whole_graph(edge_index, node_idx=None, num_hops=3, relabel_nodes=False, num_nodes=None,
...
class PGExplainer(nn.Module):
    ...
    Args:
        model (:class:`torch.nn.Module`): The target model prepared to z_explain_model
        in_channels (:obj:`int`): Number of input channels for the explanation network
        explain_graph (:obj:`bool`): Whether to z_explain_model graph classification model (default: :obj:`True`)
    ...
    def __init__(self, model, test_company_num, in_channels: int, device, epochs: int = 20, lr: float = 1e-5,
                 coff_size: float = 0.01, coff_ent: float = 5e-4, t0: float = 5.0, t1: float = 1.0,
                 sample_bias: float = 0.0, num_hops: Optional[int] = None,
                 ):
    ...
    def __set_masks__(self, x: Tensor, edge_index: Tensor, edge_mask: Tensor = None):
        ...
        .. note:: When you use the :meth:`~PGExplainer.__set_masks__`,
          the z_explain_model flag for all the :class:`torch_geometric.nn.MessagePassing`
    ...
    def __clear_masks__(self):
        """clear the edge weights to None, and set the z_explain_model flag to :obj:`False`"""
        for module in self.model.modules():
            if isinstance(module, MessagePassing):
    ...
    def get_subgraph(self, node_idx, x, edge_index, test_idx_dic, y, **kwargs, ):
        ...
        """
        num_nodes, num_edges = 6381, edge_index.size(1)
        graph = to_networkx(data=Data(x=x, edge_index=edge_index), to_undirected=True)

        subset, edge_index, new_node_mapping, edge_mask = k_hop_subgraph_with_default_whole_graph(edge_index, node_idx,
                                                                                                  self.num_hops,
                                                                                                  relabel_nodes=False,
                                                                                                  num_nodes=num_nodes,
                                                                                                  flow=self.__flow__(), )
        ...
        if y is not None:
            y = y[s]
        return x, edge_index, y, new_node_mapping, mapping, edge_mask, kwargs

    def concrete_sample(self, log_alpha: Tensor, beta: float = 1.0, training: bool = True):  # 重参数化技巧
        ...
        # 复现改过的代码
        if training:
            bias = self.sample_bias  # self.sample_bias = 0
    ...
    def explain_node(self, node_idx, x, edge_index, classifier, hete_graph, test_idx_dic, hyp_graph, y):
        select_edge_index = torch.arange(0, edge_index.shape[1])
        subgraph_x, subgraph_edge_index, _, _, mapping, subgraph_edge_mask, kwargs = self.get_subgraph(
            node_idx=node_idx, x=x, edge_index=edge_index, y=y, test_idx_dic=test_idx_dic,
            select_edge_index=select_edge_index)

        sub_idx, sub_hete_graph, sub_hyp_graph = get_sub_info(node_idx, subgraph_edge_index, subgraph_edge_mask,
                                                              hete_graph, hyp_graph, test_idx_dic, self.test_company_num)
        sub_x = x[list(mapping.keys())]
        subgraph_embed = self.model(hete_graph=sub_hete_graph, hyp_graph=sub_hyp_graph, idx=sub_idx, x=sub_x.float())
        new_node_idx = torch.tensor([sub_idx.index(node_idx)])
        _, edge_mask = self.explain(sub_x, subgraph_edge_index, sub_idx, sub_hete_graph, sub_hyp_graph, classifier,
                                    test_idx_dic, subgraph_embed, tmp=1.0, training=False, node_idx=new_node_idx,
                                    mapping=mapping)
        ...
        for i in range(len(subindices)):
            if i < edge_mask.shape[0]:
                edge_mask_full[subindices[i]] = edge_mask[i]
        return edge_mask_full

    def explain(self, x, edge_index, sub_idx, hete_graph, hyp_graph, classifier, test_idx_dic, embed, tmp=1.0,
                training=False, **kwargs, ):
        node_idx = kwargs.get("node_idx").item()
        mapping = kwargs.get("mapping")
        nodesize = embed.shape[0]
        col, row = edge_index
        c = torch.tensor([mapping[test_idx_dic[i.item()]] for i in col if test_idx_dic[i.item()] < self.test_company_num])
        r = torch.tensor([mapping[test_idx_dic[i.item()]] for i in row if test_idx_dic[i.item()] < self.test_company_num])
        f1 = embed[c]
        f2 = embed[r]
        ...
        ei = torch.stack((c, r), dim=0).to(self.device)
        mask_sparse = torch.sparse_coo_tensor(ei, values, (nodesize, nodesize))
        mask_sigmoid = mask_sparse.to_dense()
        # set the symmetric edge weights

        sym_mask = (mask_sigmoid + mask_sigmoid.transpose(0, 1)) / 2
        edge_mask = sym_mask[ei[0], ei[1]]
        # inverse the weights before sigmoid in MessagePassing Module
        self.__clear_masks__()
        self.__set_masks__(x, edge_index, edge_mask)
        company_emb = self.model(hete_graph=hete_graph, hyp_graph=hyp_graph, idx=sub_idx, x=x.float())
        logits = classifier.forward(company_emb).reshape(-1, 2)
        probs = F.softmax(logits, dim=-1)

        self.__clear_masks__()
        return probs, edge_mask

    def train_explanation_network(self, data):  # 获得要解释的全部节点的总体效果；加入 list_test_nodes
        ...
        with torch.no_grad():
            # data.to(self.device)
            self.model.eval()
            explain_node_index_list = data['list_test_nodes']
            pred_dict = {}  # 要解释的节点及其对应的预测标签
            hyp_graph = []
            for i in ['industry', 'area', 'qualify']:
                hyp_graph += [gen_attribute_hg(3976, data['hyp_graph'][i], X=None)]
            company_emb = self.model(hete_graph=data['hete_graph'], hyp_graph=hyp_graph, idx=data['idx'],
                                     x=data['features'].float())  # 获得全图的预测值
            logits = data['classifier'].forward(company_emb).reshape(-1, 2)
            for node_idx in tqdm.tqdm(explain_node_index_list):
                pred_dict[node_idx] = logits[data['test_idx_dic'][node_idx]].argmax(-1).item()
        ...
        for epoch in range(self.epochs):
            ...
            for iter_idx, node_idx in tqdm.tqdm(enumerate(explain_node_index_list)):
                with torch.no_grad():
                    x, edge_index, y, _, mapping, mask, kwargs = self.get_subgraph(node_idx=node_idx,
                                                                                   x=data['features'],
                                                                                   edge_index=data['edge_index'],
                                                                                   y=data['labels'],
                                                                                   test_idx_dic=data['test_idx_dic'])
                    sub_idx, sub_hete_graph, sub_hyp_graph = get_sub_info(node_idx, edge_index, mask,
                                                                          data['hete_graph'], data['hyp_graph'],
                                                                          data['test_idx_dic'], self.test_company_num)
                    company_emb = self.model(hete_graph=sub_hete_graph, hyp_graph=sub_hyp_graph, idx=sub_idx,
                                             x=x.float())
                    # res = data['classifier'].forward(company_emb).reshape(-1, 2)
                    new_node_idx = torch.tensor([sub_idx.index(node_idx)])
                pred, edge_mask = self.explain(x, edge_index, sub_idx, sub_hete_graph, sub_hyp_graph,
                                               data['classifier'], data['test_idx_dic'], company_emb, tmp,
                                               training=True, node_idx=new_node_idx, mapping=mapping)
                loss_tmp = self.__loss__(pred[new_node_idx.item()], pred_dict[node_idx])
                loss_tmp.backward()
                loss += loss_tmp.item()
