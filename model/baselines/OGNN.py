...
class OGNNConv(MessagePassing):
    def __init__(self, tm_net, tm_norm, hidden_dim, chunk_size):
    ...
    def forward(self, x, edge_index, last_tm_signal):
        ...
        tm_signal = tm_signal_raw.repeat_interleave(repeats=int(self.hidden_dim / self.chunk_size), dim=1)
...
class OGNN(Module):
    def __init__(self, input_dim, output_dim, company_num, person_num, n_layer=3):
        super().__init__()
        ...
        self.linear_trans_out = Linear(self.hidden_dim, output_dim)
        self.norm_input = ModuleList()
        self.ognn_layers = ModuleList()

        self.tm_norm = ModuleList()
        self.tm_net = ModuleList()

        self.linear_trans_in.append(Linear(input_dim, self.hidden_dim))

        self.norm_input.append(LayerNorm(self.hidden_dim))

        # 这两个可以随时修改
        self.dropout_rate = 0.2
        self.dropout_rate2 = 0.2

        for i in range(0):
            self.linear_trans_in.append(Linear(self.hidden_dim, self.hidden_dim))
            self.norm_input.append(LayerNorm(self.hidden_dim))

        for i in range(n_layer):
            self.tm_norm.append(LayerNorm(self.hidden_dim))
            self.tm_net.append(Linear(2 * self.hidden_dim, self.chunk_size))
            self.ognn_layers.append(OGNNConv(tm_net=self.tm_net[i], tm_norm=self.tm_norm[i], hidden_dim=self.hidden_dim,
    ...
    def forward(self, hete_graph, idx, x):
        ...
        for i in range(len(self.linear_trans_in)):
            ...
            emb = F.relu(self.linear_trans_in[i](emb))
            emb = self.norm_input[i](emb)

        tm_signal = emb.new_zeros(self.chunk_size)

        for j in range(len(self.ognn_layers)):
            if self.dropout_rate2 != 'None':
                emb = F.dropout(emb, p=self.dropout_rate2, training=self.training)
            else:
                emb = F.dropout(emb, p=self.dropout_rate, training=self.training)
            emb, tm_signal = self.ognn_layers[j](emb, edge_index, last_tm_signal=tm_signal)

        emb = F.dropout(emb, p=self.dropout_rate, training=self.training)
        emb = self.linear_trans_out(emb)
