...
class RGCN(nn.Module):
    def __init__(self, input_dim, output_dim, company_num, person_num, rel_num, n_layer=3):
        ...
        for i in range(n_layer):
            if i == 0:
                self.rgcn_layers.append(RGCNConv(input_dim, output_dim, rel_num))
            else:
                self.rgcn_layers.append(RGCNConv(output_dim, output_dim, rel_num))
    ...
    def forward(self, hete_graph, idx, x):
        ...
        for i in range(self.n_layer):
            emb = self.rgcn_layers[i](emb, edge_index, edge_type)
        return emb[idx]
